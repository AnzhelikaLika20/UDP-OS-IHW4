...
int main(int argc, char *argv[]) {
    ...
    printf("Client %d sending proposal to server.\n", client_id);
    ...
    int bytes_received = recvfrom(client_socket, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)&server_addr, &server_addr_len);
    if (bytes_received <= 0) {
        perror("recvfrom");
        close(client_socket);
        return EXIT_FAILURE;
